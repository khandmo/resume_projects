  ...
  for (int i = 1; i <= end->y - y; i++) { // SHould it be < or <=?
    ...
    for(int j = 1; j <= end->x - x; j++) {
      ...
      if (y_val == y-i) {
        ...
        if (line_func(slope, x+j-1, y-i) < y-i && line_func(slope, x+j, y-i) > y-i) {
        ...
        if (line_func(slope, x+j, y-i) > y-i && line_func(slope, x+j, y-i) < y-i-1) {
