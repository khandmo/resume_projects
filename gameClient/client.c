...
/************** main **************/
/* Main function calls all major functions to initialize the message module
* and runs the message_loop to communicate with the server.
* Also parses initial parameters to ensure a clean connection with
* an existing server.
*/
...
/************** handleInput() **************/
/* Handles keystroke input from client user.
* No major parsing involved, sends key strokes in
* designated 'KEY x' format to server
*/
static bool
...
/************** handleMessage() **************/
/* Handles messages received from server and applies the correct logic to
* ncurses so that the client user is continuously updated with
* server information. Passed through message_loop
*/
